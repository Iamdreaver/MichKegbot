...
/* 
EDITS by Jacob:
I took the example on SparkFun and updated the syntax so it works with SoftwareSerial.h instead of NewSoftSerial.  The kegbot firmware already includes SoftwareSerial.h, so this is the way to go.

Also removed code correspong to the xbee TX and RX, since we're not using that 

NOTE:  When using this, you MUST wire the ground connections of the RFID chip and the Arduino together.
*/
